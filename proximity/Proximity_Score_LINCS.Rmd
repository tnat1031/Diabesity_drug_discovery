...
```{r}
# save this
saveRDS(prox_matrix_list, "prox_matrix_list_LINCS.rds")
```
...
```{r}
prox_matrix_list <- readRDS("prox_matrix_list_LINCS.rds")
```
...
```{r}
# save
fwrite(prox_df, "proximity_scores_LINCS.txt", sep="\t")
```
