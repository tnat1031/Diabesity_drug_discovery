...
networks <- c("PPI", "GCN")
# tissues <- names(inputs$PPI)
tissues <- c("Adipose_visceral", "Liver", "Skeletal_muscle")
for (network in networks) {
    for (tissue in tissues) { # edited to skip completed tissues
        ...
        chunks <- roller::chunk_vector(seq_len(nperm), chunk_size)
...
```{r}
chunked_files <- list.files("null_matrix_chunks_LINCS", pattern="chunk.*\\.rds", full.names=T)
...
```{r}
null_files <- list.files(pattern="null_matrices_LINCS", full.names = T)
```
